...
export function generateBaccRoundResult() {
  ...
  const playerCards = [
    pokers[BaccCardPositionEnum.player_1],
    pokers[BaccCardPositionEnum.player_2],
  ];
  const bankerCards = [
    pokers[BaccCardPositionEnum.banker_1],
    pokers[BaccCardPositionEnum.banker_2],
  ];
...
//算出百家乐闲家是否补牌
function playerOutBacc(pokers: any): {
  playerIsOut: boolean;
  playerPoint: number;
} {
...
export function parseBaccResult(
  details: { banker: number[]; player: number[] },
  points: { bp: number; pp: number }
) {
  ...
  if (
    bankerPokers[0] === bankerPokers[1] ||
    playerPokers[0] === playerPokers[1]
  ) {
