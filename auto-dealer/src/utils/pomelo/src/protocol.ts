  ...
  export function encode(
    ...
    if (msgHasRoute(type)) {
      if (compressRoute) {
        ...
        if (route) {
          route = Protocol.strencode(route as string).length;
          if (route.toString().length > 255) {
            throw new Error("route maxlength is overflow");
          }
          msgLen += route.toString().length;
        }
      }
    }
...
const encodeMsgId = function (id: number, buffer: Buffer, offset: number) {
  do {
    let tmp = id % 128;
...
const encodeMsgBody = function (msg: Buffer, buffer: Buffer, offset: number) {
