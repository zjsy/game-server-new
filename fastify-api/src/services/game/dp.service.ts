...
import { calWinLoseForDp, DpDetails, parseDpResult } from '../../constants/dp.constants.js'
...
export class DpService extends GameBaseService {
  constructor (fastify: FastifyInstance) {
    super(fastify)
    console.warn('DpService initialized')
  }
  ...
  // 结算
  async settle (data: SettleRequest<DpDetails>): Promise<SettleResponse> {
    ...
    if (roundInfo.game_type !== GameType.DragonPhoenix) {
      throw new BusinessError(ErrorCode.ROUND_NOT_EXIST)
    }
    ...
    const hitRes = parseDpResult(data.details)
  ...
  // 重结算
  async reSettle (data: SettleRequest<DpDetails>): Promise<SettleResponse> {
  ...
  private async settleOrder (tempOrder: BetTempOrder, settleRoundData: SettleRoundData<DpDetails>) {
    const { winLose, rolling } = calWinLoseForDp(tempOrder.bet, settleRoundData.result, tempOrder.bet_amount)
  ...
  private async reSettleOrder (order: Pick<BetOrder, ResettleOrderField>, settleRoundData: SettleRoundData<DpDetails>) {
    const { winLose, rolling } = calWinLoseForDp(order.bet, settleRoundData.result, order.bet_amount)
    // 处理订单数据
