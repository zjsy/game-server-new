...
import { DtDetails, calRollingForDt, calWinLoseForDt, parseDtResult } from '../../constants/dt.constants.js'
import { Round } from '../../entities/RoundInfo.js'

export class DtService extends GameBaseService {
  constructor (fastify: FastifyInstance) {
    super(fastify)
    console.warn('DtService initialized')
  }
  ...
  // 结算
  async settle (data: SettleRequest<DtDetails>): Promise<SettleResponse> {
    ...
    if (roundInfo.game_type !== GameType.DragonTiger) {
      throw new BusinessError(ErrorCode.ROUND_NOT_EXIST)
    }
    ...
    let hitRes: number[] = data.result
    if (data.details) {
      updateData.details = data.details
      // 重新生成命中结果
      hitRes = parseDtResult(data.details)
    }
    ...
    for (const order of betOrders) {
      this.settleOrder(order, settleRoundData, hitRes)
    }
  ...
  // 重结算
  async reSettle (data: SettleRequest<DtDetails>): Promise<SettleResponse> {
    ...
    const settleRoundData = { round_sn: roundInfo.round_sn, result: hitRes, details: data.details, }
    for (const order of betOrders) {
      this.reSettleOrder(order, settleRoundData, hitRes)
    }
  ...
  private async settleOrder (tempOrder: BetTempOrder, settleRoundData: SettleRoundData<DtDetails>, hitRes:number[]) {
    const totalBetMoney = tempOrder.bet_amount // 下注总额
    const betDetails = tempOrder.bet
    const winLose = calWinLoseForDt(betDetails, hitRes)
    // 计算洗码
    const rolling = calRollingForDt(totalBetMoney, betDetails, hitRes)
  ...
  private async reSettleOrder (order: Pick<BetOrder, ResettleOrderField>, settleRoundData: SettleRoundData<DtDetails>, hitRes:number[]) {
