...
import { calRollingForRoulette, calWinLoseForRoulette, getRouletteResult, rouletteDetails } from '../../constants/roulette.constants.js'

export class RouletteService extends GameBaseService {
  constructor (fastify: FastifyInstance) {
    super(fastify)
    console.warn('RouletteService initialized')
  }
  ...
  // 结算
  async settle (data: SettleRequest<rouletteDetails>): Promise<SettleResponse> {
    ...
    const hitRes = getRouletteResult(data.details.n)
    ...
    this.gameRepository.updateRouletteRankingCache(roundInfo.table_id, data.details.n)
    this.gameRepository.updateRouletteStatsCache(roundInfo.table_id, data.details.n)
  ...
  // 重结算
  async reSettle (data: SettleRequest<rouletteDetails>): Promise<SettleResponse> {
  ...
  private async settleOrder (tempOrder: BetTempOrder, settleRoundData: SettleRoundData<rouletteDetails>) {
    const betDetails = tempOrder.bet
    const winLose = calWinLoseForRoulette(betDetails, settleRoundData.result)
    // 计算洗码
    const rolling = calRollingForRoulette(tempOrder.bet_amount, betDetails, settleRoundData.result)
  ...
  private async reSettleOrder (order: Pick<BetOrder, ResettleOrderField>, settleRoundData: SettleRoundData<rouletteDetails>) {
    const betDetails = order.bet
    const winLose = calWinLoseForRoulette(betDetails, settleRoundData.result)
    // 计算洗码
    const rolling = calRollingForRoulette(order.bet_amount, betDetails, settleRoundData.result)
    // 处理订单数据
