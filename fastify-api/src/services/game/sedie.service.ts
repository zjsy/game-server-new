...
import { calWinLoseForSeDie, generateSeDieResult, seDieDetails } from '../../constants/sedie.constants.js'

export class SeDieService extends GameBaseService {
  constructor (fastify: FastifyInstance) {
    super(fastify)
    console.warn('SeDieService initialized')
  }
  ...
  // 结算
  async settle (data: SettleRequest<seDieDetails>): Promise<SettleResponse> {
    ...
    if (roundInfo.game_type !== GameType.SeDie) {
      throw new BusinessError(ErrorCode.ROUND_NOT_EXIST)
    }
    ...
    const hitRes = generateSeDieResult(data.details.rc)
  ...
  // 重结算
  async reSettle (data: SettleRequest<seDieDetails>): Promise<SettleResponse> {
  ...
  private async settleOrder (tempOrder: BetTempOrder, settleRoundData: SettleRoundData<seDieDetails>) {
    const betDetails = tempOrder.bet
    const winLose = calWinLoseForSeDie(betDetails, settleRoundData.result)
    // 计算洗码,色碟的大小单双都有抽水,所以都计入洗码
  ...
  private async reSettleOrder (order: Pick<BetOrder, ResettleOrderField>, settleRoundData: SettleRoundData<seDieDetails>) {
