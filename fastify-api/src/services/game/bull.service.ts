...
import { BullDetails, calWinLoseForBull } from '../../constants/bull.constants.js'
import { Round } from '../../entities/RoundInfo.js'

export class BullService extends GameBaseService {
  constructor (fastify: FastifyInstance) {
    super(fastify)
    console.warn('BullService initialized')
  }
  ...
  // 结算
  async settle (data: SettleRequest<BullDetails>): Promise<SettleResponse> {
    ...
    if (roundInfo.game_type !== GameType.Bull) {
      throw new BusinessError(ErrorCode.ROUND_NOT_EXIST)
    }
    ...
    const settleRoundData = { round_sn: roundInfo.round_sn, result: data.result, details: data.details }
  ...
  // 重结算
  async reSettle (data: SettleRequest<BullDetails>): Promise<SettleResponse> {
  ...
  private async settleOrder (tempOrder: BetTempOrder, settleRoundData: SettleRoundData<BullDetails>) {
    const betDetails = tempOrder.bet
    const { winLose, rolling } = calWinLoseForBull(betDetails, settleRoundData.result, settleRoundData.details)
  ...
  private async reSettleOrder (order: Pick<BetOrder, ResettleOrderField>, settleRoundData: SettleRoundData<BullDetails>) {
