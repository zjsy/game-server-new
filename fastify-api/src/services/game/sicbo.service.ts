...
import { calRollingForSicBo, calWinLoseForSicBo, parseSicBoResult, sicBoDetails } from '../../constants/sicbo.constants.js'

export class SicBoService extends GameBaseService {
  constructor (fastify: FastifyInstance) {
    super(fastify)
    console.warn('SicBoService initialized')
  }
  ...
  // 结算,约定sicbo结果已经是从小到大排列的三个骰子点数
  async settle (data: SettleRequest<sicBoDetails>): Promise<SettleResponse> {
    ...
    if (roundInfo.game_type !== GameType.SicBo) {
      throw new BusinessError(ErrorCode.ROUND_NOT_EXIST)
    }
    ...
    const hitRes = parseSicBoResult(data.details)
  ...
  // 重结算
  async reSettle (data: SettleRequest<sicBoDetails>): Promise<SettleResponse> {
  ...
  private async settleOrder (tempOrder: BetTempOrder, settleRoundData: SettleRoundData<sicBoDetails>) {
    const betDetails = tempOrder.bet
    const winLose = calWinLoseForSicBo(betDetails, settleRoundData.result, settleRoundData.details)
    // 计算洗码
    const rolling = calRollingForSicBo(tempOrder.bet_amount, betDetails)
  ...
  private async reSettleOrder (order: Pick<BetOrder, ResettleOrderField>, settleRoundData: SettleRoundData<sicBoDetails>) {
    const betDetails = order.bet
    const winLose = calWinLoseForSicBo(betDetails, settleRoundData.result, settleRoundData.details)
    // 计算洗码
    const rolling = calRollingForSicBo(order.bet_amount, betDetails)
    // 处理订单数据
